...
public class ScrollList : MonoBehaviour
{
    ...
    public event Action<ScrollListItem> renderItemEvent;
    ...
    private void OnEnable()
    {
        ...
        {
            throw new Exception("[Scroll Rect] 组件不存在");
        }
    ...
    public void Init<T>() where T : ScrollListItem
    {
        ...
        if (m_ScrollRect is null)
        {
            throw new Exception("ScrollRect组件为空");
        }

        if (m_ScrollRect.content is null || m_ScrollRect.viewport is null)
        {
            throw new Exception("ScrollRect组件设置异常");
        }

        if (prefab is null)
        {
            throw new Exception("prefab为空");
        }
    ...
    public void SetItemCount(int count, bool keepPosition = true)
    {
        if (!m_IsInit)
        {
            throw new Exception("未初始化，必须先调用Init方法进行初始化");
        }
