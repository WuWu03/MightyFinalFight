...
public class TalkView : UIBaseView<TalkComponent, TalkSettings>
{
    ...
    private void OnItemUpdateEvent(TalkComponent.TalkSelectItem item)
    {
        TalkConfigData talkConfigData = ConfigDataSheet.talkConfigDatas.GetConfigDataById(m_TalkId);
        item.languageSelect.SetLanguageTextKey(talkConfigData.talkSelect[item.itemIndex].content);
    }

    private void OnItemSelectEvent(TalkComponent.TalkSelectItem item, bool isSelect)
    {
        if (isSelect)
        {
