...
namespace GameFrameWork.UI
{
    ...
    {
        private static readonly Dictionary<Type, GameFrameWorkFunc<IUIView>> s_Factories = new();
        private static readonly Dictionary<string, Type> s_ViewTypes = new();

        public static IUIView GetUIView(Type uiType)
        {
            if (s_Factories.TryGetValue(uiType, out GameFrameWorkFunc<IUIView> factory))
            {
                return factory();
            }

            throw new GameFrameWorkException("未找到对应的UI类型 : [" + uiType.Name + "]，请生成对应的UI类或UIFactory文件");
        }
        ...
        // 运行时允许覆盖（便于测试/DI）
        public static void RegisterFactory(Type uiType, GameFrameWorkFunc<IUIView> factory)
        {
