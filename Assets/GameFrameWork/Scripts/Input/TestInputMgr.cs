...
public class InputEvent : GameFrameWorkEventArg
{
    ...
    public void Add(GameFrameWorkAction<InputEventType> inputEvent)
    {
        if (m_Event is GameFrameWorkAction<InputEventType> events)
        {
            events += inputEvent;
        }
    }

    public void Add<InputValueType>(GameFrameWorkAction<InputValueType, InputEventType> inputEvent) where InputValueType : struct
    {
        if (m_Event is GameFrameWorkAction<InputValueType, InputEventType> events)
        {
            events += inputEvent;
        }
    }
    ...
    {
        if (m_Event is GameFrameWorkAction<InputEventType> inputCallback)
        {
    ...
    {
        if (m_Event is GameFrameWorkAction<InputValueType, InputEventType> inputCallback)
        {
...
{
    public event GameFrameWorkAction<InputDevice, InputDeviceChange> inputDeviceChangeEvent
    {
        add { m_InputDeviceChangeEvent += value; }
        remove { m_InputDeviceChangeEvent -= value; }
    }

    private event GameFrameWorkAction<InputDevice, InputDeviceChange> m_InputDeviceChangeEvent;
    ...
    public void Init(IResourcesMgr resourceMgr, string configDataName, string saveKey)
    {
        ...
        {
            string configDataPath = GameFrameWorkEntry.config.configDataPath;
    ...
    public void SetCurrScheme(string schemeName)
    {
        ...
        {
            throw new GameFrameWorkException("配置文件不存在");
        }

        m_CurrActionMap = m_InputActionAsset.FindActionMap(schemeName);
    }

    public void AddInputEvent(string actionName, GameFrameWorkAction<InputEventType> inputCall)
    {
        //this.AddInputEvent(actionName, inputCall, null);
    }

    public void AddInputEvent<InputValueType>(string actionName, GameFrameWorkAction<InputValueType, InputEventType> inputCall) where InputValueType : struct
    {
    ...
    private bool CanAddInputEvent(string actionName)
    {
        ...
        {
            throw new GameFrameWorkException("输入方案不存在");
        }
        ...
        if (inputAction == null)
        {
            throw new GameFrameWorkException("输入映射不存在");
        }
    ...
    private void InvokeInputEvent(InputAction action, InputEventType inputEventType)
    {
        ...
        {
            throw new GameFrameWorkException("输入事件不存在");
        }
