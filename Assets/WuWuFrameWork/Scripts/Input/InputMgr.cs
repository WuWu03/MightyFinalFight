...
namespace WuWuFramework.Input
{
    ...
    public class InputMgr : WuWuFrameworkModule , IInputMgr
    {
        ...
        private event WuWuFrameworkAction m_InputDeviceChangeEvent;
        ...
        public void AddAfterTriggerEvent(KeyType keyType, WuWuFrameworkAction afterTriggerEvent)
        {
            ...
            eventList = new List<WuWuFrameworkAction>() { afterTriggerEvent };
        ...
        private void CheckCombo()
        {
            ...
            if (m_KeyDownTimer > 0 && Time.time - m_KeyDownTimer >= KeyDownTime)
            {
                if (!isTrigger && m_CurrKeyDown >= 0)
                {
                    ...
                    if (m_DicAfterTriggerEvents.TryGetValue(keyType, out List<WuWuFrameworkAction> triggerEvents))
                    {
