...
namespace WuWuFramework.BehaviourTree
{
    public class BehaviourTree
    {
        ...
        {
            Type t = Type.GetType("WuWuFramework.BehaviourTree." + className) ?? Type.GetType(className);

            if (t == null)
            {
                throw new WuWuFrameworkException(StringUtil.Append("行为树数据实例不存在 : [", id.ToString(), "]"));
            }
