...
public class FindReferences01 : EditorWindow
{
    ...
    public static void FindThread(string path, bool outExcel = false)
    {
        ...
        if (!string.IsNullOrEmpty(path))
        {
            ...
            string[] files = Directory.GetFiles(Path.Combine(Application.dataPath, "ArtResources"), "*.*", SearchOption.AllDirectories)
        ...
        if (showNoUser)
        {
            ...
            foreach (var item in refDic)
            {
                ...
                if (item.Value.Count == 0)
                {
                    ...
                    GUILayout.Label(assetPath, GUILayout.Width(800));
                    EditorGUILayout.ObjectField("", assetObj, typeof(Object), true, GUILayout.Width(200));
