...
namespace WuWuFramework.Editor
{
    public class AssetBundleBuilder : IDisposable
    {
        ...
        private bool GenerateBuildMap(AssetBundleConfig config)
        {
            ...
            //框架资源
            AddSingleBuildMap("Materials", new List<string>() { "Assets/WuWuFramework/Materials/" }, ".assetbundle", "*", config.listDatas.Count);
            AddSingleBuildMap("Shaders", new List<string>() { "Assets/WuWuFramework/Shaders/" }, ".assetbundle", "*", config.listDatas.Count + 1);
