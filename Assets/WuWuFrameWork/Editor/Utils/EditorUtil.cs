...
namespace WuWuFramework.Editor
{
    public static class EditorUtil
    {
        ...
        public static Type[] GetAssemblyTypes(string typeName, params string[] parttern)
        {
            ...
            foreach (Type type in allTypes)
            {
                if (type.GetInterfaces().Contains(baseType))
                {
                    ...
                    {
                        if (parttern[i].Contains(type.Name) || type.Name.Contains(parttern[i]))
                        {
                            isParttern = true;
                            break;
                        }
        ...
        private static void CreateScriptableObject(Type type, string name, string ext, string dir = null)
        {
            ...
            {
                directory = PathUtil.GetAssetFullPath(WuWuFramework.Editor.EditorMgr.GetWuWuFrameworkConfig().configDataPath);
            }
